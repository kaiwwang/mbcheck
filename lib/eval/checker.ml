...
let rec check_and_update_mailboxes (program,comp,stack) mailbox_reference app_list =
  ...
  match comp,stack with
    | Return _ ,[] -> 
        Hashtbl.fold (fun key_list count acc ->
            if List.mem mailbox_reference key_list then count else acc
        ) mailbox_counting 0       
    ...
    | Let {binder = _; term; cont},stack ->
        check_and_update_mailboxes (program,term,(Frame (mailbox_reference,[],cont)) :: stack) mailbox_reference app_list 
    ...
    | Seq (comp1, comp2),  stack ->
        check_and_update_mailboxes (program, comp1,  Frame (mailbox_reference,[],comp2) :: stack) (mailbox_reference) app_list 

    | Return v,  Frame (new_mailbox_reference, _, cont) :: stack ->
        if (check_var v mailbox_reference) then begin 
            Hashtbl.iter (fun key_list count ->
                if List.mem mailbox_reference key_list then
                    Hashtbl.replace mailbox_counting key_list (count+1)
            ) mailbox_counting;
        end 
        else ();
        check_and_update_mailboxes (program, cont,  stack) new_mailbox_reference app_list 
    ...
    | App {func; args},  stack -> 
        ...
        | Variable (func_var, _) ->
          ...
          if List.exists (fun (f_var, f_args) -> f_var = func_var && f_args = args) app_list then begin 
            check_and_update_mailboxes (program,Return (Constant (Unit)), stack) mailbox_reference app_list end
        else
            ...
            | Some func_decl ->
                let updated_mailbox_reference =
                  ...
                  | indexed_args ->
                      ...
                      | Some (index, _) -> 
                        ...
                        let key_with_mailbox_reference =
                          Hashtbl.fold (fun key count acc ->
                              if List.mem mailbox_reference key then Some (key, count) else acc
                          ) mailbox_counting None
                        in
                        (match key_with_mailbox_reference with
                        | Some (key, count) ->
                            if not (List.mem binder key) then begin
                                let new_key = binder :: key in
                                Hashtbl.remove mailbox_counting key;
                                Hashtbl.add mailbox_counting new_key (count+1)
                            end
                            else
                                Hashtbl.replace mailbox_counting key (count+1);
                            binder
                        | None ->
                            mailbox_reference)
                      | None -> 
                ...
                in
                check_and_update_mailboxes (program, func_decl.decl_body, stack) updated_mailbox_reference app_list'
    ...
    | If {test = _; then_expr; else_expr},  stack -> 
        check_and_update_mailboxes (program, then_expr,  Frame (mailbox_reference,[],else_expr) :: stack) mailbox_reference app_list 
    ...
    | Send {target; message = _; _},  stack ->
        if (check_var target mailbox_reference) then begin 
            Printf.printf "+3\u{1F535}";
            Hashtbl.iter (fun key_list count ->
                if List.mem mailbox_reference key_list then
                    Hashtbl.replace mailbox_counting key_list (count+1)
            ) mailbox_counting;
...
let process (program,comp,stack) mailbox_reference = 
    check_and_update_mailboxes (program,comp,stack) mailbox_reference []
